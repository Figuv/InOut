...
const Tasks = (props) => {
  ...
  const getCoordinatorTeams = async () => {
    const q = query(
      collection(db, "teams"),
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      const teams = await getCoordinatorTeams();
    ...
    <View className="bg-[#fff] h-full w-full items-center">
      ...
      <View className="border-[#e7e7e6] border rounded w-11/12 h-5/6 my-5 shadow p-1">
        ...
        <View className="h-20 flex-row justify-around items-center border-b-2 border-[#e7e7e6]">
          {/*Agregar tareas*/}
          <TouchableOpacity
            className="w-full h-full flex-col md:flex-row items-center justify-center py-3"
            onPress={() => {
          ...
          >
            ...
            <View className="ml-1">
              <Text className="text-[#232323] text-sm md:text-base font-bold">
                Añadir Tarea
              </Text>
            </View>
          </TouchableOpacity>
